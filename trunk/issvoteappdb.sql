...
BEGIN
    SELECT COUNT(*) INTO i FROM USER_CATALOG
        WHERE TABLE_NAME = 'TO_THEO_DOI_SQ' AND TABLE_TYPE = 'SEQUENCE';
    IF i = 0 THEN
        EXECUTE IMMEDIATE 'CREATE SEQUENCE "TO_THEO_DOI_SQ"';
    END IF;
END;
/

CREATE OR REPLACE TRIGGER "TO_THEO_DOI_TR"
BEFORE INSERT ON "TO_THEO_DOI"
FOR EACH ROW
WHEN (new."ID" IS NULL)
    BEGIN
        SELECT "TO_THEO_DOI_SQ".nextval
        INTO :new."ID" FROM dual;
    END;
/
